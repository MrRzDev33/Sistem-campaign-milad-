...
import { Mail, Lock, Loader2, Phone, Eye, EyeOff, User as UserIcon, Store } from 'lucide-react';
...
export default function LoginPage() {
  ...
  React.useEffect(() => {
    const saved = localStorage.getItem('dmc_kasir_remember_me');
    if (saved) {
      try {
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('Invalid login credentials')) {
          throw new Error('No HP atau password salah');
        }
        throw error;
      }
      ...
      if (data.user) {
        if (rememberMe) {
          localStorage.setItem('dmc_kasir_remember_me', JSON.stringify({ phone, password, rememberMe }));
        } else {
          localStorage.removeItem('dmc_kasir_remember_me');
        }
  ...
  return (
    <div className="min-h-screen bg-orange-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[40px] shadow-2xl overflow-hidden border border-orange-100 animate-in fade-in zoom-in duration-300">
        <div className="bg-orange-500 p-10 text-center relative overflow-hidden">
          ...
          />
          <p className="text-orange-100 text-sm font-medium">Pencatatan Penjualan Donat Madu Cihanjuang</p>
        </div>
        
        <div className="p-8">
          <div className="mb-8 text-center">
            <h2 className="text-2xl font-black text-gray-900">Login Kasir Outlet</h2>
            <p className="text-gray-500 text-sm">Masuk untuk mencatat transaksi outlet</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700 ml-1">No HP Kasir</label>
            ...
            <div className="flex flex-col gap-4 items-center">
              <p className="text-sm text-gray-500">
                Belum punya akun?{' '}
                <Link to="/register-kasir" className="text-orange-600 font-bold hover:underline">
                  Daftar Sekarang
                </Link>
              </p>
            </div>
