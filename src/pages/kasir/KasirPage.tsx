...
export default function KasirPage() {
  ...
  useEffect(() => {
    ...
    // Listen for product price changes to update local catalog immediately
    const priceChannel = supabase
      .channel('public:product_prices:kasir')
  ...
  const handleSaveTransaction = async () => {
    ...
    try {
      ...
      // CartItem extends Product sehingga is_loyalty sudah tersedia langsung
      // Gunakan Boolean() agar lebih robust terhadap null/undefined/1
      const hasLoyalty = items.every(item => Boolean(item.is_loyalty));
      const hasRegular = items.every(item => !item.is_loyalty);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <button
                  key={product.id}
                  onClick={() => {
                    // Gunakan is_loyalty langsung dari CartItem (sudah mengandung data Product)
                    const hasLoyaltyInCart = items.some(i => i.is_loyalty === true);
                    const hasRegularInCart = items.some(i => !i.is_loyalty);
      ...
      <div className={cn(
              ...
              <div key={item.id} className="flex items-center gap-4 group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {item.is_loyalty && (
                      <span className="bg-yellow-100 text-yellow-700 text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase">Promo</span>
                    )}
                  </div>
        ...
        <div className="p-6 border-t bg-gray-50 space-y-6 overflow-y-auto max-h-[60%]">
          ...
          {items.length > 0 && (
            <div className={cn(
              ...
              items.every(item => Boolean(item.is_loyalty)) 
                ? "bg-yellow-50 border-yellow-100 text-yellow-800" 
                : "bg-blue-50 border-blue-100 text-blue-800"
            )}>
              <div className="flex items-center gap-2">
                <Ticket className="w-5 h-5" />
                <span className="text-[10px] font-black uppercase tracking-wider">Tipe Transaksi:</span>
              </div>
              <span className="text-xs font-black uppercase">
                {items.every(item => Boolean(item.is_loyalty)) ? 'Promo Milad' : 'Reguler'}
              </span>
            </div>
          )}
          ...
          {/* Reminder foto semua produk - di atas tombol simpan agar selalu terlihat */}
          {items.length > 1 && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3">
              ...
              <p className="text-xs font-bold text-amber-700 leading-snug">
                Ada <span className="underline">{items.length} produk</span> di keranjang. Pastikan foto struk mencakup <span className="underline">semua produk</span> yang dibeli!
              </p>
            </div>
          )}
