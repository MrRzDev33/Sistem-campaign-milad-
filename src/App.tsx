...
export default function App() {
  ...
  const fetchUserProfile = async (userId: string, retryCount = 0) => {
    ...
    try {
      ...
      if (data) {
        // Validasi tambahan: Jika kasir tapi tidak punya outlet, anggap profil rusak
        if (data.role === 'kasir' && !data.outlet_id) {
          console.warn('Kasir tanpa outlet ditemukan:', userId);
          toast.error('Profil kasir Anda belum lengkap (tidak ada outlet). Silakan hubungi Admin untuk reset akun.');
          handleLogout('/login');
          return;
        }
        ...
        toast.error('Akun Anda ditemukan tapi profil kasir belum terdaftar. Silakan hubungi Admin atau coba daftar ulang.');
        // Force sign out to clear the broken session
        handleLogout('/login');
      }
    } catch (error: any) {
